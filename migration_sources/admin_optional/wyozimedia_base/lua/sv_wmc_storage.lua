...
function wyozimc.AddMedia(link, by)
	...
	provider.QueryMeta(udata, function(data)
		...
		by:ChatPrint("Media added! " .. data.Title)
...
net.Receive("wyozimc_list", function(le, cl)
	...
	wyozimc.Debug("MediaCount: ", mediacount)

	local iteration_size = 40
	local iterations = math.max(1, math.floor(mediacount / iteration_size))
