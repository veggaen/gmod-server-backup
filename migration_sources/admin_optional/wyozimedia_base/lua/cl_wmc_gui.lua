...
local function SearchYoutube(q, callback)
...
local function FindURL(text, callback)
	text = text:Trim()
	...
	if playnetmsg then
		if wyozimc.HasPermission(LocalPlayer(), "PlayAll") then
			menu:AddOption("Play", function()
				net.Start(playnetmsg) net.WriteString(theurl) net.WriteEntity(passent) net.SendToServer()
				frame:Close()
		...
		menu:AddOption("Play", function()
			wyozimc.PlayUrl(theurl, _, bit.band(playflags, wyozimc.FLAG_PLAYING_CLIENTSIDE))
			wyozimc.ChatText(Color(255, 127, 0), "[MediaPlayer] ", Color(255, 255, 255), "Playing ", Color(252, 84, 84), theurl, Color(255, 255, 255), " locally.")
		end):SetIcon( "icon16/music.png" )
		...
		if permission_add or permission_playall or playnetmsg then
			...
			if permission_add then
				...
				local function SendAddData()
					local state, err = FindURL(addnewentry:GetText(), function(link)
				...
				--[[do
					...
					luckbtn.DoClick = function()
						SearchYoutube(addnewentry:GetText(), function(theurl)
							net.Start("wyozimc_edit")
								net.WriteString("add")
								net.WriteString(theurl)
							net.SendToServer()
			...
			addnewinfo:SetTooltip([[You don't have to enter a link! Nonlinks automatically add the first found video on YouTube.]])
...
net.Receive("wyozimc_list", function()
	...
	table.Add(wyozimc.AllLines, net.ReadTable())
...
hook.Add("HUDPaint", "WyoziMCDefaultHUD", function()
	if not wyozimc.ShowHUD then return end
	...
	local hh = wyozimc.FlashFound and 50 or 70
	...
	local t = FormatSimpleTime(goneseconds, "%02i:%02i")
	...
	if not wyozimc.FlashFound and wyozimc.Provider and not wyozimc.Provider.UseGmodPlayer then
